...
abstract Doudecimal_Image( Image8Struct ) from Image8Struct to Image8Struct {
    ...
    **/
    ...
    public function new( w: Int, h: Int ){
       this = ( 
        { width:  w, height: h
        , image:  new haxe.io.UInt8Array( Std.int( w * h * 3  ) ) 
        }: Image8Struct
        );
    }
    /*
    ...
    **/
    inline
    public function pos3( x: Int, y: Int, ?off: Int = 0 ): Int
        return Std.int( position( x, y ) * 3  + off ); 
    /**
    ...
    **/
    ...
    public inline function set_doudecimalPixel( x: Float, y: Float, newColor: Doudecimal_Color ): Doudecimal_Color {
        ...
        if( newColor.alpha < 1.-small && this.transparent ){
            var pos:Int = pos3( Std.int( x ), Std.int( y ) );
            var c1 = this.image[pos];
            var c2 = this.image[pos+1];
            var c3 = this.image[pos+2];
            var currColor = new Doudecimal_Color();
            currColor.setIntDirect( Doudecimal_Color.fromChannels( c1, c2, c3 ) );
            currColor.blend( newColor );
            this.image[pos]   = currColor.c1;
            this.image[pos+1] = currColor.c2;
            this.image[pos+2] = currColor.c3;
        } else {
            var pos:Int = pos3( Std.int( x ), Std.int( y ) );
            this.image[pos]   = newColor.c1;
            this.image[pos+1] = newColor.c2;
            this.image[pos+2] = newColor.c3;
        }
        return newColor;
    }
    /**
    ...
    **/
    ...
    public function drawToContext( ctx: js.html.CanvasRenderingContext2D, x: Int, y: Int  ){
        var data = new js.lib.Uint8ClampedArray( Std.int( 4*Std.round( length/3 ) ) );
        var pos: Int = 0;
        ...
        for( h in 0...height ){
            for( w in 0...width ){
                ...
                var c2 = image[ pos+1 ];
                var c3 = image[ pos+2 ];
                // recompose Doudecimal_Color
                d.c1 = c1;
                d.c2 = c2;
                d.c3 = c3;
                // extract 0xFF FF FF FF and set them on the Uint8Array.
                i = w * h * 4;
                data[i]     = d.alpha;
			    data[i + 1] = color.red;
			    data[i + 2] = color.green;
			    data[i + 3] = color.blue;
            }
        }
